...
def get_html(url):
    r = open(url, 'r')
    return r.read()
with open ('testAdv.csv','a+', newline="") as file:
    writer = csv.writer(file)
    writer.writerow(['User','Number','Product','AdvType','Place','Date'])
    for i in range(41,42):
        ...
        for tag in soup.find_all('div', {'class':'message'} and {'class':'default'} and {'class':'clearfix'}):
            ...
            if (adv_info):    
                adv_dict[user] = [user] + adv_info + [data]
                writer.writerow(adv_dict.get(user))
