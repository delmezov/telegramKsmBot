...
def example_message(message):
    ...
    bot.send_photo(message.chat.id, open('content/example.png', 'rb'), config.exmple_message,  parse_mode = "HTML")
...
def handler_new_member(message):
    if message.chat.type != "private" and message.chat.id == bot.get_chat(config.chat_name).id:
        bot.delete_message(message.chat.id,message.message_id)
        print(bot.get_chat(config.chat_name).id)
        #user_name = message.new_chat_member.first_name
        #bot.send_message(message.chat.id, config.greeting_message_public.format(user_name), parse_mode = "HTML") Приветствие при входе в группу 
...
@bot.message_handler(func=lambda message: message.entities is not None and message.chat.id == bot.get_chat(config.chat_name).id)
...
def sticker_checker(message):
    if message.chat.id == bot.get_chat(config.chat_name).id:
        bot.delete_message(message.chat.id, message.message_id)
        bot.restrict_chat_member(bot.get_chat(config.chat_name).id, message.from_user.id, until_date=time()+60)
...
def end_poll(message):
    ...
    if isinstance(message.text, str) and len(message.text) < 80:
        user_dict[message.chat.id].append(message.text)
        print(user_dict)
        bot.send_message(message.chat.id, "Ожидайте, идёт подготовка объявления!")
...
def delete_keybord(message):
    ...
    if message.text == 'Всё верно!':  
        ...
        url_button = types.InlineKeyboardButton(text="Перейти на объявление!", url="https://t.me/agro_zerno")
        keyboard.add(url_button)

        keyboard1 = types.InlineKeyboardMarkup()
        url_button1 = types.InlineKeyboardButton(text="Написать объявление", url="https://t.me/agro_zerno")
        keyboard1.add(url_button1)

        bot.send_message(message.chat.id, config.last_message, parse_mode = "HTML", reply_markup = keyboard)
        
        #bot.send_photo(bot.get_chat(config.chat_name).id, open(text_on_image.text_on_image_(user_dict[message.chat.id][0], user_dict[message.chat.id][1], user_dict[message.chat.id][2], user_dict[message.chat.id][3], user_dict[message.chat.id][4]), 'rb'), config.who_send_message.format(message.from_user.first_name, message.from_user.last_name, message.from_user.username), parse_mode = "HTML", reply_markup=keyboard1)
        bot.send_photo(bot.get_chat(config.chat_name).id, open(text_on_image.text_on_image_(user_dict[message.chat.id][0], user_dict[message.chat.id][1], user_dict[message.chat.id][2], user_dict[message.chat.id][3], user_dict[message.chat.id][4]), 'rb'), parse_mode = "HTML", reply_markup=keyboard1)

        #bot.send_message(message.chat.id, config.last_message, parse_mode = "HTML", reply_markup = types.ReplyKeyboardRemove())

       #bot.send_message(bot.get_chat(config.chat_name).id, adv_functions.list_to_string(user_dict[message.chat.id]) + config.who_send_message.format(message.from_user.first_name, message.from_user.last_name, message.from_user.username), parse_mode = "HTML")
